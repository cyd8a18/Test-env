...
def factory(name, **kwargs):
    ...
    if name.startswith('pb_cook_meal_'):
        ...
        if kwargs['robot'] == 'ur5':
            return pb_cook_meal_ur5(**kwargs)
        else:
...
def kitchen_scene():
    ...
    veggies = ['cabbage', 'pumpkin', 'tomato']
...
def pb_cook_meal(task_file, robot, start_task, num_tasks, num_episodes, num_ingredients, num_dishes, max_ingredient_per_dish,
                 shuffle, seed=0):
    ...
    print("dish_i: ", dish_i)
    print("dish_ings: ", dish_ings)
    print("goal: ", goal[dish_i])
    print("subgoals: ", subgoals[ing_i])
    print("dependencies: ", dependencies[ing_i])


#   MODIFIED ENVIRONMENT FOR UR5 ----

def kitchen_scene_ur5():
    objects = [
        dict(path=URDFS['short_floor'], type_name='table', fixed=True, globalScaling=0.8),
    ]
    object_types = ['table']
    ...
    for o_type in containers:
        n_copy = copies.get(o_type, 1)
        for i in range(n_copy):
            placements.append(('set', o_type + '/%i' % i, 'serving'+ str(i+1) +'/0', [[-0.45 + 0.45 * i, 0.6, 0.5], [0, 0, 0, 1]])) #np.array([[-0.4, -0.45], [0.4, 0.45]])))

    for o_type in ingredients:
        n_copy = copies.get(o_type, 1)
        for i in range(n_copy):
            placements.append(('on', o_type + '/%i' % i, 'table/0', np.array([[-0.4, -0.45], [0.4, 0.45]])))
...
#   MODIFIED PROBLEM FOR UR5 ----

def pb_cook_meal_ur5(task_file, robot, start_task, num_tasks, num_episodes, num_ingredients, num_dishes, max_ingredient_per_dish,
                 shuffle, seed=0):
...
def main():
    ...
    pgen = list(factory('pb_cook_meal_3i_2d_3m_iter',
                        task_file=None,
                        start_task=0,
                        num_tasks=-1,
                        num_episodes=1,
