...
import numpy as np
from third_party.pybullet.utils.pybullet_tools.utils import wait_for_duration, get_body_name
...
class TaskEnv(object):
    ...
    def execute_command(self, action, time_step=0.0):
        ...
        if 'ur5' in get_body_name(command.body_paths[0].body):
            conf, max_joints, min_joints = command.refine().execute(time_step=time_step)
        else:
        ...
        if 'ur5' in get_body_name(command.body_paths[0].body):
            return integrity, max_joints, min_joints
        else:
            return integrity
