...
def plan_pick(robot, target, grasp_gen, fixed, digits, teleport, resolutions=None):     #digits: tactile sensor
    ik_fn = get_ik_fn(robot, digits, fixed=fixed, teleport=teleport, resolutions=resolutions)
    free_motion_fn = get_free_motion_gen(robot, digits, fixed=([target] + fixed), teleport=teleport, resolutions=resolutions)
...
def plan_place(robot, holding, target, place_gen, grasp, fixed, digits, teleport, conf0=None, resolutions=None):
    ik_fn = get_ik_fn(robot, digits, fixed=fixed, teleport=teleport, resolutions=resolutions)
    holding_motion_fn = get_holding_motion_gen(robot, digits, fixed=fixed, teleport=teleport, resolutions=resolutions)
...
class ActionPlanner(object):
    ...
    def plan(self, action_name, object_args, cont_args=None, teleport=False):
        ...
        if action_name == 'pick':
            ...
            # NEW: Check objects fixed
            fixed = [obj.uid for obj in self.world.fixed.all_types]

            plan = plan_pick(
                self.world.robot, object_args[0], grasp_gen, fixed, self.world.digits,   # Tacto inclusiton
                teleport=teleport,
                resolutions=resolutions
            )

            # ORIGINAL: 
            # Problem: This expression self.world.fixed.ids is always [] 
            # plan = plan_pick(
            ...
            plan = plan_place(
                ...
                place_gen, self._state.pop('grasp'), self.world.fixed.ids, self.world.digits,   # Tacto inclusiton
                teleport=False,
                resolutions=resolutions
