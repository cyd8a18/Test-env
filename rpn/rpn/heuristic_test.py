...
from third_party.pybullet.utils.pybullet_tools.utils import WorldSaver, connect, dump_world, get_bodies_in_region, \
    get_center_extent, get_pose, set_pose, Pose, \
    ...
    get_lower_upper, get_custom_limits, draw_pose 
...
def test_env(args):
    ...
    for ingredient in ingredients[2:]:
        ...
        if args.random == True:
            world.random_place(world.id(ingredient+'/0'), world.id('table/0'), np.array([[-0.3, -0.1], [0.1, 0.4]]))#([[-0.3, -0.2], [0.3, 0.2]]))
        else:
    ...
    for trial in range(args.trials):
        ...
        with pb_session(use_gui=True):
            ...
            for ingredient in ingredients:
                position, orientation = get_pose(world.id(ingredient+'/0'))
                approach_pos = (position[0], position[1], position[2]+0.1)
                x, y, z = approach_pos

                draw_pose((approach_pos,orientation))
                world.move_cartesian_space(x, y, z, 0, 0)
                time.sleep(2)
