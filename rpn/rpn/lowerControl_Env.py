...
#p.addUserDebugLine([-0.3, -0.1, 0.05],  [0.1, 0.4, 0.05], [1, 0, 0], 4)
AREA_ING = [[-0.2, -0.1], [0.05, 0.4]]
...
class Test_World(World):
    ...
    def load_tacto(self):
        links_ids = [self.joints['left_inner_finger_pad_joint'].id, self.joints['right_inner_finger_pad_joint'].id]

        self.digits = tacto.Sensor(width=120, height=160, visualize_gui=True)
...
def test_env(args):
    ...
    for ingredient in ingredients[2:]:
        ...
        if args.random == True:
            world.random_place(world.id(ingredient+'/0'), world.id('table/0'), np.array(AREA_ING))#([[-0.3, -0.2], [0.3, 0.2]]))
        else:
    ...
    #draw_square(AREA_ING)

    return world

def draw_square(diagonal):
    lower_pnt , upper_pnt = diagonal
    xl, yl = lower_pnt
    xu, yu = upper_pnt

    p.addUserDebugLine([xl, yl, 0.05],  [xl, yu, 0.05], [1, 0, 0], 4)
    p.addUserDebugLine([xl, yu, 0.05],  [xu, yu, 0.05], [1, 0, 0], 4)
    p.addUserDebugLine([xu, yl, 0.05],  [xu, yu, 0.05], [1, 0, 0], 4)
    p.addUserDebugLine([xl, yl, 0.05],  [xu, yl, 0.05], [1, 0, 0], 4)



def main():
    parser = argparse.ArgumentParser(description='Teaching environment configuration')
    parser.add_argument('--robot', default='ur5', type=str, choices=["ur5","drake"])
    parser.add_argument('--scale', default=1, type=float)
    parser.add_argument('--trials', default=1, type=int)
    parser.add_argument('--ing_num', default=0, type=int)
    parser.add_argument('--test', default="separate", type=str, choices=["separate","group"])
    parser.add_argument('--random', default=False, dest="random", action="store_true")
    ...
    for trial in range(args.trials):
        ...
        with pb_session(use_gui=True):
            ...
            if args.test == "separate": #For new world per ingredient
                ingredients = [ingredients[args.ing_num]]  
            ...
            for ingredient in ingredients:
                ...
                for action_name, action_args, action_plan, ret in goal_to_motion_plan(goal2Test, env, False):
                    ...
                    if 'ur5' in get_body_name(world.robot):
