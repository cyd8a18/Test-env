...
self.joints, self.controlGripper, self.controlJoints, self.mimicParentName =\
            setup_sisbot(robotID, gripper_type)
...
GRIPPER_GRASPED_LIFT_HEIGHT = 1.2 / 10
...
def move_gripper(controlGripper, gripper_opening_length: float, step: int = 120):
...
def close_gripper(robot, joints, controlGripper, controlJoints, mimicParentName, step: int = 120, check_contact: bool = False) -> bool:
    ...
    for step_idx in range(1, step):
        ...
        move_gripper(controlGripper, current_target_open_length, 1)
    return False
