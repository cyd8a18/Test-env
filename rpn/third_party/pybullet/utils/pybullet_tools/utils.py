...
from third_party.pybullet.utils.models.ur5.utilities import Models, setup_sisbot
...
# UR5 ADJUSMENT VARIABLES
GRASP_POINT_OFFSET_Z = 0.1
...
def set_joint_positions(body, joints, values):
    # if 'ur5' not in get_body_name(body):
    assert len(joints) == len(values)
    for joint, value in zip(joints, values):
        set_joint_position(body, joint, value)
    # else:
    #     valuesUR5 = values[0:7]
    #     assert len(joints) == len(valuesUR5)
    #     for joint, value in zip(joints, values):
    #         set_joint_position(body, joint, value)
...
def get_movable_joints(body): # 45 / 87 on pr2
    # Modify for ur5 model
    if 'ur5' in get_body_name(body):
        jointsInfo, _, controlJoints, _ = setup_sisbot(body, get_body_name(body).split('_')[-1])
        movable_joints_ids = [jointsInfo[joint].id for joint in jointsInfo if joint in controlJoints]
        return movable_joints_ids[:-1]  # Filter out the gripper
...
def get_sample_fn(body, joints, custom_limits={}):
    ...
    if 'ur5' in get_body_name(body):
        lower_limits, upper_limits = ([-2, -2, -2, -2, -2, -2],[2, 1, 2, 2, 2, 2.9])
    def fn():
...
def grasp_pose_offset(robot, grasp_pose, offset):
    if 'ur5' in get_body_name(robot):
        gripper_pose_z = grasp_pose[0][2]+offset
        new_gripper_pose = ((grasp_pose[0][0],grasp_pose[0][1],gripper_pose_z), grasp_pose[1])
        return new_gripper_pose
    return grasp_pose
...
def inverse_kinematics_helper(robot, link, target_pose):
    ...
    if 'ur5' in get_body_name(robot):
        ...
        if target_quat is None:
            kinematic_conf = p.calculateInverseKinematics(robot, link, target_point, physicsClientId=CLIENT, jointDamping=jd) #, \
                #     #maxNumIterations=100)
        else:
            kinematic_conf = p.calculateInverseKinematics(robot, link, target_point, target_quat, physicsClientId=CLIENT, jointDamping=jd) #, \
                #     #maxNumIterations=100)
...
def is_pose_close(pose, target_pose, pos_tolerance=1e-3, ori_tolerance=1e-3*np.pi):
    ...
    if (target_quat is not None) and not np.allclose(quat, target_quat, atol=ori_tolerance, rtol=0):
        ...
        #print("Diff: ", abs(np.subtract(quat,target_quat)))
        return False
    return True
...
def inverse_kinematics(robot, link, target_pose, max_iterations=200, custom_limits={}, **kwargs):
    ...
    for iterations in range(max_iterations):
        ...
        if 'ur5' in get_body_name(robot):
            ...
            # pose_adjusment = ((target_pose[0][0], target_pose[0][1], target_pose[0][2] + GRASP_POINT_OFFSET_Z), \
            #     target_pose[1]) 
            # # target_pose[0][2] = target_pose[0][2] + pose_adjusment
            
            _, _, yaw = p.getEulerFromQuaternion(target_pose[1])
            orn_adjustment = p.getQuaternionFromEuler([np.pi,np.pi/2,yaw])
            # draw_pose((target_pose[0], orn_adjustment))

            kinematic_conf = inverse_kinematics_helper(robot, link, (target_pose[0], orn_adjustment))
            #kinematic_conf = inverse_kinematics_helper(robot, link, target_pose)
            kinematic_conf = kinematic_conf[0: len(movable_joints)]

            if kinematic_conf is None:
                return None
            set_joint_positions(robot, movable_joints, kinematic_conf)
            #if is_pose_close(get_link_pose(robot, link), (pose_adjusment[0], orn_adjustment), **kwargs):
            # draw_pose(get_link_pose(robot, link))
            
            if is_pose_close(get_link_pose(robot, link), (target_pose[0], orn_adjustment), **kwargs):

                # Angle constraints for UR5
                lower_limits = [-2, -2,    -2,   -2.4,   -1.6,   -4.1]
                upper_limits = [ 2,  2,   2.4,   -0.7,   -1.5,    2.5]
                break
        else:
            kinematic_conf = inverse_kinematics_helper(robot, link, target_pose)
        ...
        # print("NEVER FINDS A CLOSE POSITION")
        return None
    # lower_limits, upper_limits = get_custom_limits(robot, movable_joints, custom_limits)
    
    
    if not all_between(lower_limits, kinematic_conf, upper_limits):
        # print("return None because of limits")
        return None
    return kinematic_conf
