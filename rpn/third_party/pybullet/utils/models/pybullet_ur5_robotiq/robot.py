...
class RobotBase(object):
    ...
    def reset_arm(self):
        ...
        for rest_pose, joint_id in zip(self.arm_rest_poses, self.arm_controllable_joints):
            ...
            p.setJointMotorControl2(self.id, joint_id, p.POSITION_CONTROL, targetPosition=rest_pose,
...
class UR5Robotiq85(RobotBase):
    def __init_robot__(self):
        ...
        #../third_party/pybullet/utils/models/pybullet_ur5_robotiq
        self.id = p.loadURDF('../third_party/pybullet/utils/models/pybullet_ur5_robotiq/urdf/ur5_robotiq_85_digit.urdf', self.base_pos, self.base_ori,
                             useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES)
    ...
    # New for gripper control --- Start
    def open_gripper(self):
        self.move_gripper(self.gripper_range[1])

    def move_gripper(self, open_length, force = None, modification = False):
        # Force definition, important for the tactile sensor
        if force == None:
        ...
        if modification == False:
            ...
            p.setJointMotorControl2(self.id, self.mimic_parent_id, p.POSITION_CONTROL, targetPosition=open_angle,
                                    force=force, maxVelocity=self.joints[self.mimic_parent_id].maxVelocity)
            # ORIGINAL --- End
        else:
            ...
            for _ in range(120):
                p.setJointMotorControl2(self.id, self.mimic_parent_id, p.POSITION_CONTROL, 
                                        targetPosition=open_angle, force=2, 
                                        maxVelocity=self.joints[self.mimic_parent_id].maxVelocity)
                
                for child_joint_id in self.mimic_child_multiplier.keys():
                    child_pose = open_angle * self.mimic_child_multiplier[child_joint_id]
                    
                    if force == None:
                        force = self.joints[child_joint_id].maxForce
                    
                    p.setJointMotorControl2(self.id, child_joint_id, p.POSITION_CONTROL, 
                                        targetPosition=child_pose, force=2, 
                                        maxVelocity=self.joints[child_joint_id].maxVelocity)
                
                p.stepSimulation()

    def close_gripper(self, force = None, steps = 2):
        initial_position = p.getJointState(self.id, self.mimic_parent_id)[0]
        initial_position = math.sin(0.715 - initial_position) * 0.1143 + 0.010
        for step in range(1, steps):
            current_target_open_length = initial_position - step / steps * initial_position
            current_target_open_length = np.clip(current_target_open_length,*self.gripper_range)

            if self.gripper_contact(force=force) == True:
                return True

            self.move_gripper(current_target_open_length, force = force, modification = True)
    ...
    def gripper_contact(self, force=100):
        contact_left  = p.getContactPoints(bodyA=self.id, linkIndexA=self.left_pad_id)
        contact_right = p.getContactPoints(bodyA=self.id, linkIndexA=self.right_pad_id)

        if not (contact_right and contact_left):
            return False

        # Check the force
        left_force  = p.getJointState(self.id, self.left_pad_id)[2][:3]  # 6DOF, Torque is ignored
        right_force = p.getJointState(self.id, self.right_pad_id)[2][:3]
    ...
    def add_force_sensors(self):
        self.left_pad_id  = 12
        self.right_pad_id = 17
        # Add force sensors
        p.enableJointForceTorqueSensor(self.id, self.left_pad_id)
        p.enableJointForceTorqueSensor(self.id, self.right_pad_id)
        # Change the friction of the gripper 
        p.changeDynamics(self.id, self.left_pad_id, lateralFriction=0.5)
        p.changeDynamics(self.id, self.right_pad_id, lateralFriction=0.5)
