...
from third_party.pybullet.utils.models.ur5.utilities import Models, setup_sisbot, setup_sisbot_force, Camera
from third_party.pybullet.utils.pybullet_tools.utils import all_between
...
class ClutteredPushGrasp:
    ...
    def __init__(self, models: Models, camera: Camera, vis=False, num_objs=3, gripper_type='85') -> None:
        ...
        self.tablaID = p.loadURDF(str(PATH_ur5 / "urdf/objects/table.urdf"),
                                  [0.0, -0.5, 0.8],
                                  p.getQuaternionFromEuler([0, 0, 0]),
                                  useFixedBase=True)
        self.UR5StandID = p.loadURDF(str(PATH_ur5 / "urdf/objects/ur5_stand.urdf"),
                                     [-0.7, -0.36, 0.0],
                                     p.getQuaternionFromEuler([0, 0, 0]),
                                     useFixedBase=True)
        self.robotID = p.loadURDF(str(PATH_ur5) + "/urdf/ur5_robotiq_" + gripper_type + ".urdf",
        ...
        self.joints, self.controlGripper, self.controlJoints, self.mimicParentName =\
            setup_sisbot(self.robotID, gripper_type)
    ...
    def step(self, position: tuple, angle: float, action_type: str, debug: bool = False):
        ...
        #print("Coordinates from sliders: ", x,"  ",y, "  ", z)
        ...
        print("Coordinates after debug/no debug: ", x,"  ",y, "  ", z)
    ...
    def move_ee(self, action, max_step=1, check_collision_config=None, custom_velocity=None,
        ...
        for _ in range(max_step):
            ...
            # # JOINTS         o   o   o   o   o   o     x      
            # lower_limits = [-2, -2, -2, -2.055, -1.6, -3,   -2] 
            # upper_limits = [ 2,  2,  2, -0.685, -1.5, -2.6,  2]               
            
            # if all_between(lower_limits, joint_poses[0:7], upper_limits):
            ...
            self.step_simulation()
            # contact_left = p.getContactPoints(bodyA=self.robotID, linkIndexA=self.joints['left_inner_finger_pad_joint'].id)
            # print(contact_left)
            # print(self.gripper_contact())
...
class CustomEnvironment(ClutteredPushGrasp):
    def __init__(self,models,camera,vis,num_objs,gripper_type):
        super().__init__(models,camera,vis,num_objs,gripper_type)
        #(self, models: Models, camera: Camera, vis=False, num_objs=3, gripper_type='85')
    
    def test(self):
        print("this is an inheritance test")