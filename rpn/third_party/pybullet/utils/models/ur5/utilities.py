...
def setup_sisbot(robotID, gripper_type):
    ...
    def controlGripper(robotID, parent, children, mul, **kwargs):
        ...
        # Force and velocity values
        empiric_force = 2.5
        # nuckle_force = 10
        # finger_force = 10
        empiric_velocity = 1.0
        # ------------
        if controlMode == p.POSITION_CONTROL:
            ...
            p.setJointMotorControl2(robotID, parent.id, controlMode, targetPosition=pose,
                                    force=empiric_force, maxVelocity=empiric_velocity) #parent.maxForce, maxVelocity=parent.maxVelocity)
            # move child joints
            for name in children:
                # if "knuckle_joint" in name:
                #     empiric_force = nuckle_force
                #     empiric_velocity = 2.0
                # else:
                #     empiric_force = finger_force
                #     empiric_velocity = 1.0

                child = children[name]
                childPose = pose * mul[child.name]
                p.setJointMotorControl2(robotID, child.id, controlMode, targetPosition=childPose,
                                        force=empiric_force, maxVelocity=empiric_velocity) #force=child.maxForce, maxVelocity=child.maxVelocity)
        
        elif controlMode == p.VELOCITY_CONTROL:
            pose = kwargs.pop("targetPosition")
            targetVel = 1
            p.setJointMotorControl2(robotID, parent.id, controlMode, targetPosition=pose,
                                    targetVelocity=targetVel, force=parent.maxForce)  
            for name in children:
                child = children[name]
                childPose = pose * mul[child.name]
                p.setJointMotorControl2(robotID, child.id, controlMode, targetPosition=childPose,
                                        targetVelocity=targetVel, force=child.maxForce)
...
def setup_sisbot_force(robotID, gripper_type):
    controlJoints = ["shoulder_pan_joint", "shoulder_lift_joint",
                     "elbow_joint", "wrist_1_joint",
                     "wrist_2_joint", "wrist_3_joint",
                     "finger_joint"]
